...
class EffectEditorBasicSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasOwnAsset =
        CameraEffectRenderTypeApi.isSticker(form.renderType) ||
        CameraEffectRenderTypeApi.isNone(form.renderType);
      ...
      child: Column(
        ...
        children: [
          ...
          FeFormColorPicker(
            ...
            allowClear: true,
            onSelected: (hex) => bloc.add(EffectPreviewColorChanged(hex)),
          ),
          ...
          FeEditorSyncedTextField(
            ...
            onChanged: (value) => bloc.add(
              EffectPreviewColorChanged(value.trim().isEmpty ? null : value),
            ),
          ),
          ...
          const SizedBox(height: 10),
          FeEffectRenderTypeField(
            value: form.renderType,
            onChanged: (value) =>
                bloc.add(EffectBasicFieldChanged(renderType: value)),
          ),
          if (_fieldError(context, 'renderType') != null) ...[
            const SizedBox(height: 4),
            Text(
              _fieldError(context, 'renderType')!,
              style: Theme.of(
                context,
          ...
          if (isDistortion) ...[
            const SizedBox(height: 10),
            FeDistortionPresetField(
              value: form.distortionPreset,
              errorText: _fieldError(context, 'distortionPreset'),
              onChanged: (value) => bloc.add(
                EffectBasicFieldChanged(
                  distortionPreset: value,
                  clearDistortionPreset: value == null,
                ),
              ),
            ),
          ],
          if (hasOwnAsset) ...[
            ...
            const SizedBox(height: 10),
            FeEditorSyncedTextField(
              value: form.assetAsset ?? '',
              decoration: InputDecoration(
                labelText: l10n.tOr('feFieldAssetAsset', 'Bundled asset'),
                hintText: l10n.tOr(
                  'feFieldAssetAssetHint',
                  'Bundled asset path on the mobile client',
                ),
                errorText: _fieldError(context, 'assetAsset'),
              ),
              onChanged: (value) => bloc.add(
                EffectBasicFieldChanged(
                  assetAsset: value,
                  clearAssetAsset: value.trim().isEmpty,
                ),
              ),
            ),
          ],
...
class _EffectAssetUploadField extends StatelessWidget {
  ...
  Future<void> _pickAndUpload(BuildContext context) async {
    ...
    context.read<EffectEditorBloc>().add(
      UploadEffectAssetEvent(bytes: picked.bytes, filename: picked.name),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final error = state.fieldErrors['assetUrl'] ?? state.fieldErrors['asset'];
