...
class NotificationsBloc
  ...
  NotificationsBloc({
    ...
    on<NotificationScheduleUpdated>(_onScheduleUpdated);
    on<ScheduleNotificationRequested>(_onScheduleRequested);
    on<CancelScheduledNotification>(_onCancelScheduled);
    on<_ProcessDueScheduledNotifications>(
      (event, emit) => _processDueScheduledNotifications(emit),
    );
  }
  ...
  void _onScheduleModeChanged(
    NotificationScheduleModeChanged event,
    Emitter<NotificationsState> emit,
  ) {
    emit(
      state.copyWith(
  ...
  void _onScheduleUpdated(
    NotificationScheduleUpdated event,
    Emitter<NotificationsState> emit,
  ) {
    emit(
      state.copyWith(
  ...
  Future<void> _onScheduleRequested(
    ...
    final scheduledAt = state.scheduledDateTime;
    if (scheduledAt == null || !scheduledAt.isAfter(DateTime.now())) {
      emit(
        state.copyWith(
          status: NotificationsSendStatus.error,
          errorMessage: 'Scheduled time cannot be in the past.',
        ),
      );
      return;
    }
    ...
    final request = NotificationRequestEntity(
      userId: event.userId,
      userIds: event.userIds,
      title: event.title,
      body: event.body,
    ...
    ScheduledNotificationEntity scheduled;
    try {
      ...
      scheduled = ScheduledNotificationEntity(
        id: '${DateTime.now().millisecondsSinceEpoch}_${event.title.hashCode}',
        scheduledAt: scheduledAt,
        timezoneName: timezoneName,
        title: event.title,
        body: event.body,
    ...
    final updatedQueue = [scheduled, ...state.scheduledNotifications];
    emit(
      state.copyWith(
        status: NotificationsSendStatus.scheduled,
        scheduledNotifications: updatedQueue,
        isScheduled: false,
  ...
  void _onCancelScheduled(
    CancelScheduledNotification event,
    Emitter<NotificationsState> emit,
  ) {
    final updated = state.scheduledNotifications
        .map(
          (item) => item.id == event.id
              ? item.copyWith(status: ScheduledNotificationStatus.cancelled)
              : item,
        )
        .toList(growable: false);
    emit(state.copyWith(scheduledNotifications: updated));
  }

  void _ensureScheduleTimer() {
    _scheduleTimer ??= Timer.periodic(
      const Duration(seconds: 30),
      (_) => add(const _ProcessDueScheduledNotifications()),
    );
  }

  Future<void> _processDueScheduledNotifications(
    Emitter<NotificationsState> emit,
  ) async {
    final now = DateTime.now();
    final due = state.pendingScheduledNotifications
    ...
    for (final item in due) {
      try {
        ...
        switch (item.target) {
          case ScheduledNotificationTarget.single:
            if (item.userId != null) {
              await sendNotification(request.copyWithUserId(item.userId!));
            }
          case ScheduledNotificationTarget.bulk:
            if (item.userIds != null && item.userIds!.isNotEmpty) {
              await sendBulkNotification(
                request.copyWithUserIds(item.userIds!),
              );
            }
          case ScheduledNotificationTarget.broadcastAll:
            await broadcastNotification(request);
          case ScheduledNotificationTarget.broadcastAdmins:
            await broadcastAdminsNotification(request);
        }

        final updated = state.scheduledNotifications
            .map(
              (entry) => entry.id == item.id
                  ? entry.copyWith(status: ScheduledNotificationStatus.sent)
                  : entry,
            )
            .toList(growable: false);
        emit(state.copyWith(scheduledNotifications: updated));
        ...
        final updated = state.scheduledNotifications
            .map(
              (entry) => entry.id == item.id
                  ? entry.copyWith(status: ScheduledNotificationStatus.failed)
                  : entry,
            )
            .toList(growable: false);
        emit(state.copyWith(scheduledNotifications: updated));
  ...
  DateTime _defaultScheduledDateTime() {
    final now = DateTime.now();
...
extension on NotificationRequestEntity {
  NotificationRequestEntity copyWithUserId(String userId) =>
      NotificationRequestEntity(
        userId: userId,
        userIds: userIds,
        title: title,
  ...
  NotificationRequestEntity copyWithUserIds(List<String> userIds) =>
      NotificationRequestEntity(
        userId: userId,
        userIds: userIds,
        title: title,
...
class _ProcessDueScheduledNotifications extends NotificationsEvent {
  const _ProcessDueScheduledNotifications();
}
