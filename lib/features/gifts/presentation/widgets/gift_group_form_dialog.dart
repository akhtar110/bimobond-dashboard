...
class GiftGroupFormDialog extends StatefulWidget {
  const GiftGroupFormDialog({super.key, this.group});

  final GiftGroupEntity? group;

  bool get isEditing => group != null;

  static Future<GiftGroupFormResult?> show(
    BuildContext context, {
    GiftGroupEntity? group,
  }) {
    return showDialog<GiftGroupFormResult>(
      context: context,
      builder: (_) => GiftGroupFormDialog(group: group),
    );
  }

  @override
  State<GiftGroupFormDialog> createState() => _GiftGroupFormDialogState();
}

class GiftGroupFormResult {
  GiftGroupFormResult.create(this.createData) : updateData = null;
  GiftGroupFormResult.update(this.updateData) : createData = null;

  final CreateGiftGroupData? createData;
  final UpdateGiftGroupData? updateData;
}

class _GiftGroupFormDialogState extends State<GiftGroupFormDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      title: Text(
        widget.isEditing
            ? l10n.tOr('giftGroupEditTitle', 'Edit tab')
            : l10n.tOr('giftGroupAddTitle', 'Add tab'),
      ),
      content: SizedBox(
        width: 420,
        child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                TextFormField(
                  ...
                  decoration: InputDecoration(
                    labelText: l10n.tOr('giftGroupSlug', 'Slug'),
                    helperText: l10n.tOr('giftGroupSlugHint', 'Unique, lowercase'),
                  ),
                  validator: (v) =>
                      v == null || v.trim().isEmpty ? l10n.t('requiredField') : null,
                ),
                const SizedBox(height: 12),
                Text(
                  l10n.tOr('giftGroupIconUrl', 'Tab icon'),
                ...
                TextFormField(
                  ...
                  decoration: InputDecoration(
                    labelText: l10n.tOr('giftGroupSortOrder', 'Sort order'),
                  ),
                ),
                const SizedBox(height: 8),
                SwitchListTile(
                  contentPadding: EdgeInsets.zero,
                  title: Text(l10n.tOr('giftStatusActive', 'Active')),
                  value: _isActive,
